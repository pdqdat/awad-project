...
exports.markAsFavorite = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error marking as favorite", error: error.message });
  }
};
...
exports.rateMovie = async (req, res) => {
  ...
  try {
    ...
    const existingRating = user.ratingList.find(
    ...
    if (existingRating) {
      ...
      const adjustedVoteAverage =
        (currentVoteAverage * currentVoteCount - oldRating + rating) /
        currentVoteCount;
      ...
      const newVoteAverage =
        (currentVoteAverage * currentVoteCount + rating) /
        (currentVoteCount + 1);
...
exports.getWatchlist = async (req, res) => {
  ...
  try {
    ...
    // Lấy danh sách ID theo thứ tự thêm vào
    const sortedWatchlist = user.watchlist.sort(
    ...
    // Sắp xếp kết quả theo thứ tự của `movieIds`
    const orderedMovies = movieIds.map((id) =>
      movies.find((movie) => movie.tmdb_id === id)
    );

    res.status(200).json({ watchlist: orderedMovies });
  } catch (error) {
    res
      .status(500)
...
exports.getFavoriteList = async (req, res) => {
  ...
  try {
    ...
    const sortedFavoriteList = user.favoriteList.sort(
      (a, b) => new Date(a.addedAt) - new Date(b.addedAt)
    );
    ...
    res.status(200).json({ favoriteList: orderedMovies });
} catch (error) {
    res
      .status(500)
      .json({ message: "Error fetching watchlist", error: error.message });
...
exports.getRatingList = async (req, res) => {
    ...
    try {
        ...
        const sortedRatingList = user.ratingList.sort((a, b) => new Date(a.ratedAt) - new Date(b.ratedAt));
        ...
        const orderedMovies = movieIds.map(id => {
            ...
            const userRating = sortedRatingList.find(item => item.movieId === id).rating;
        ...
        res.status(200).json({ ratingList: orderedMovies });
    } catch (error) {
...
exports.removeFromFavoriteList = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error removing movie from favorite list",
        error: error.message,
      });
  }
};
...
exports.removeFromRatingList = async (req, res) => {
  ...
  try {
    ...
    const ratingIndex = user.ratingList.findIndex(
      (item) => item.movieId === movieId
    );
    if (ratingIndex === -1) {
      return res
        .status(404)
        .json({ message: "Rating not found in rating list" });
    }
    ...
    if (currentVoteCount > 1) {
      const newVoteAverage =
        (currentVoteAverage * currentVoteCount - oldRating) /
        (currentVoteCount - 1);
      movie.vote_average = newVoteAverage;
      movie.vote_count = currentVoteCount - 1;
    } else {
    ...
    res
      .status(500)
      .json({ message: "Error removing rating", error: error.message });
  }
};
...
exports.getPersonalLists = async (req, res) => {
  ...
  try {
    const lists = await PersonalList.find({ userId }).sort({ createdAt: 1 });
...
exports.getPersonalListById = async (req, res) => {
  ...
  try {
    ...
    const sortedMovies = list.movies.sort(
      (a, b) => new Date(a.addedAt) - new Date(b.addedAt)
    );

    const movieIds = sortedMovies.map((item) => item.movieId);
    const movies = await Movie.find(
      { tmdb_id: { $in: movieIds } },
      {
        _id: 1,
    ...
    res
      .status(500)
      .json({ message: "Error fetching list details", error: error.message });
  }
};
